...
# MAGIC
# MAGIC #### Overview
# MAGIC This notebook creates curated **Delta tables** in the `gold` schema of the `dwh_project` catalog, following a **star schema** design for analytics.  
# MAGIC
# MAGIC Instead of temporary views, the Gold layer is materialized as persistent **tables**, which improves performance, consistency, and integration with BI/reporting tools.  
# MAGIC
# MAGIC #### Tables Created
# MAGIC
# MAGIC - **`dwh_project.gold.dim_customers`**  
# MAGIC   Contains customer attributes enriched from CRM and ERP sources, with surrogate key `customer_key`.  
# MAGIC
# MAGIC - **`dwh_project.gold.dim_products`**  
# MAGIC   Contains product information with categories, subcategories, and costs, with surrogate key `product_key`.  
# MAGIC
# MAGIC - **`dwh_project.gold.fact_sales`**  
# MAGIC   Fact table capturing sales transactions, linking to `dim_customers` and `dim_products`.  
# MAGIC
# MAGIC #### Data Cleansing
# MAGIC After joins, each DataFrame applies **`.dropna()`** to remove records containing null values. This ensures clean and consistent data in the Gold layer.  
# MAGIC
# MAGIC
# MAGIC
...
def create_gold_tables():
    ...
    print('================================================')
    print('Creating Gold Layer Tables (with nulls dropped)')
    print('================================================')
    ...
    try:
        ...
        dim_customers_df.write.format("delta").mode("overwrite").saveAsTable("dwh_project.gold.dim_customers")
        ...
        dim_products_df.write.format("delta").mode("overwrite").saveAsTable("dwh_project.gold.dim_products")
        ...
        fact_sales_df.write.format("delta").mode("overwrite").saveAsTable("dwh_project.gold.fact_sales")
        ...
        print('   - Total Duration: ' + str(int(batch_end_time - batch_start_time)) + ' seconds')
        print('==========================================')
