...
# MAGIC ####Purpose
# MAGIC
# MAGIC This PySpark script validates the Gold Layer tables (dim_customers, dim_products, fact_sales) in a Spark/Databricks environment to ensure data integrity and readiness for analytics. Checks include:
# MAGIC
# MAGIC Uniqueness of surrogate keys (customer_key, product_key) in dimension tables.
# MAGIC Referential integrity between fact_sales and dimension tables, addressing potential NULLs in foreign keys.
# MAGIC
# MAGIC #### Usage
# MAGIC
# MAGIC Run: Execute gold_quality_checks.py in a Databricks notebook or PySpark environment after populating the Gold Layer. Ensure SparkSession is active and tables are accessible (e.g., gold schema).
# MAGIC Review: Check console output (via .show() and counts) for issues like duplicate keys or unmatched fact records.
# MAGIC Resolve: Fix flagged issues (e.g., orphaned keys) via ETL updates or by adding "unknown" dimension rows before downstream analytics.
# MAGIC
